...
std::string CreateNewFileNameReal() {
...
std::string CreateNewFileNamefake() {
      ...
      while (active_.load(std::memory_order_relaxed)) {
          ...
          while (join_.load(std::memory_order_acquire) && queue->front()) { // if stop thread it will still save all outstanding logs in the queue if join is true
            ...
            if(NewDay != LastDay){
              ...
              std::string newFileName = CreateNewFileNamefake();
...
int main(){
  ...
  std::string fname = CreateNewFileNameReal(); 
  logg.SetOutput(fname);

  for(int64_t i = 0;i<1000;i++){
    logg.Log("test", i);
    //logg.stop_and_join(); // by this time writer thread spins inside second while loop
  }

  std::string expectedFilename = CreateNewFileNamefake(); 
